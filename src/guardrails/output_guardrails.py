...
def content_filter(response: str) -> dict:
    ...
    PII_PATTERNS = {
        ...
        "Database connection": r"(mongodb|mysql|postgresql)://[^\s]+",
...
class OutputGuardrailPlugin(base_plugin.BasePlugin):
    ...
    async def after_model_callback(
        ...
        *,
        ...
        # Apply LLM safety check if enabled
        if self.use_llm_judge:
            judge_result = await llm_safety_check(response_text)
            if not judge_result["safe"]:
                self.blocked_count += 1
                # Replace with safe message
                safe_message = "🚫 This response was flagged as potentially unsafe and has been blocked for your protection."
